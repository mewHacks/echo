...
describe('ServerState Module', () => {
    ...
    describe('checkTriggers()', () => {
        ...
        it('should detect HELP_REQUEST from voice summary safety keywords', () => {
            const safetyKeywords = [
                'stalker', 'stalking', 'harassment',
                'suicide', 'self-harm',
                'call the police', 'danger'
            ];

            for (const keyword of safetyKeywords) {
                const state = createMockState({
                    lastVoiceSummary: `User mentioned something about ${keyword} in conversation`,
                });
    ...
    describe('getServerState()', () => {
        ...
        it('should expire voice summary if older than VOICE_EXPIRATION_MS', async () => {
            ...
            // Voice summary should be nulled out due to expiration
            expect(state.lastVoiceSummary).toBeNull();
            expect(state.lastVoiceTimestamp).toBeNull();
        });
...
describe('ServerState Edge Cases', () => {
    ...
    it('should handle case-insensitive safety keyword detection', () => {
        const state = createMockState({
            lastVoiceSummary: 'User mentioned HARASSMENT in caps',
        });
    ...
    it('should handle partial keyword matches in voice summary', () => {
        const state = createMockState({
            lastVoiceSummary: 'Discussing harassment policies at work',
        });
